...
function SectionLabel({ children }: { children: string }) {
  return (
    <div className="font-sans mb-1.5 text-[10px] font-bold tracking-[0.08em] text-[#9bc2ff] sm:text-[11px]">
      {children}
    </div>
  );
}
    ...
    <SlideShell wide className="gap-1.5">
      ...
      <Anim className="shrink-0">
        <div className="rounded-xl border border-[#0056d2]/20 bg-[#0056d2]/10 px-3 py-1.5">
          <span className="font-sans text-[10px] font-bold text-[#9bc2ff] sm:text-[11px]">
            هدف ·{" "}
          </span>
          <span className="font-sans text-[clamp(0.78rem,1.15vw,0.92rem)] leading-relaxed text-white">
            {slide.goal}
          </span>
        </div>
      </Anim>
      ...
      {slide.funFact ? (
        <Meta>
          <span className="font-sans font-semibold text-[#9bc2ff]">
            آیا می‌دانستید؟{" "}
          </span>
          {slide.funFact}
        </Meta>
