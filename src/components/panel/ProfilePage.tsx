...
import { masterclass, profileAchievements, studentProfile } from "@/lib/mock-data";

const learningFields = [
  { label: "دوره", value: studentProfile.program },
  { label: "مدرس", value: masterclass.teacher },
  { label: "سطح", value: studentProfile.level },
  { label: "زمان کلاس", value: `پنجشنبه · ${masterclass.timeShort}` },
] as const;
...
export function ProfilePage() {
  ...
  return (
    <div className="space-y-6 pb-4 sm:space-y-8">
      <PageHeader
        eyebrow="هنرجوی مسترکلاس"
        title="پروفایل هنرجو"
        description="اطلاعات شما در پنل اختصاصی مسترکلاس تئوری موسیقی استاد دهقانیار."
      />
      ...
      >
        ...
        <div className="relative flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
            ...
            <div className="text-center sm:text-right">
              ...
              <p className="mt-1.5 text-sm text-white/80">
                {studentProfile.program} · سطح {studentProfile.level}
              </p>
          ...
          <div className="grid grid-cols-3 gap-2 sm:gap-3">
            {[
              { label: "حضور", value: studentProfile.attendanceRate },
              { label: "جلسات", value: `${studentProfile.totalHours}` },
              { label: "دوره", value: studentProfile.activeCourses },
            ].map((item) => (
              <div
                key={item.label}
