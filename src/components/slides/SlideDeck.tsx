...
import { slides, toFa, TOTAL_SLIDES } from "@/lib/session-1-slides";
...
export function SlideDeck({
  ...
  const { index, direction, next, prev, goTo } = useDeck();
        ...
        <div className="mx-auto flex h-14 w-full max-w-360 items-center justify-between gap-3 px-4 sm:h-16 sm:px-6">
          <div className="flex min-w-0 items-center gap-3">
            {mode === "fullscreen" ? (
              ...
              <Link
                href="/dashboard/sessions/1/play"
                className="inline-flex items-center gap-1.5 rounded-full border border-white/10 bg-white/4 px-3 py-1.5 text-xs text-white/80 hover:bg-white/8"
              >
            ...
            <div className="min-w-0">
              <p className="font-display text-sm font-bold">اتود</p>
              <p className="truncate text-[11px] text-white/60">جلسهٔ اول</p>
            </div>
          </div>
          <div className="font-display flex items-baseline gap-2 rounded-full border border-white/8 bg-white/3 px-3 py-1.5 text-sm tabular-nums">
            <span>{toFa(String(index + 1).padStart(2, "0"))}</span>
            <span className="text-white/15">/</span>
            <span className="text-[#8ba0c0]">{toFa(TOTAL_SLIDES)}</span>
          </div>
        </div>
        ...
        <div className="flex w-full max-w-360 items-center gap-1.5 rounded-full border border-white/10 bg-white/4 px-1.5 py-1 backdrop-blur-md sm:gap-2 sm:px-2">
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-center justify-center gap-1.5">
              <span className="font-display shrink-0 text-[11px] font-bold tabular-nums">
                {toFa(index + 1)}
          ...
          <select
            ...
            className="font-display max-w-22 shrink-0 truncate rounded-full border border-white/10 bg-white/4 px-2 py-0.5 text-[10px] text-gray-400 outline-none hover:bg-white/7"
          >
            {slides.map((s, i) => (
